...
export async function fetchAnime(year: number, season: string): Promise<string> {
    return post<string>('/api/admin/fetch/anime', null, { params: { year: year.toString(), season } });
}
...
export async function fetchMapping(year: number, season: string): Promise<string> {
    return post<string>('/api/admin/fetch/mapping', null, { params: { year: year.toString(), season } });
}
